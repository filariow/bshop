...
import (
	...
	"github.com/filariow/bshop/pkg/bill/http/rest"
	"github.com/filariow/bshop/pkg/bill/mocks"
...
func Test_Createbill(t *testing.T) {
	...
	ttc := []testCase{
		...
		{
			name: "Minimum valid bill details",
			q: request{
				Name:  "Name",
				Price: 1.0,
			},
	...
	tf := func(t *testing.T, tc *testCase) {
		...
		r := mocks.NewMockbillRepository(ctrl)
		s := rest.New(r)
		...
		q := httptest.NewRequest(http.MethodPost, "/bills", toJSON(t, &tc.q))
		cr := chi.NewRouter()
		cr.Route("/bills", func(nr chi.Router) {
			s.RegisterRoutes(nr)
		})
		cr.ServeHTTP(w, q)
